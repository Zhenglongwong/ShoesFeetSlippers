...
import Pagination from "../../pages/Landing/Pagination";
import { getByRoleName } from "../utilities";

describe("Pagination (daisyUI)", () => {
	describe("ui", () => {
		test("should have 3 buttons", () => {
			render(<Pagination currentPage={0} lastPage={3} setFetchPage={() => {}} />);
			expect(screen.getAllByRole("button")).toHaveLength(3);
		});
		test("should display the current page out of the total pages", () => {
			render(<Pagination currentPage={1} lastPage={3} setFetchPage={() => {}} />);
	...
	// 		const user = userEvent.setup();
	// 		render(<Pagination currentPage={2} lastPage={3} setFetchPage={() => {}} />);
	// 		await user.click(getByRoleName("button", "»"));
